...
public class AiModelFactoryImpl implements AiModelFactory {
    ...
    @Override
    public OpenAiAudioTranscriptionModel getOrCreateOpenAiAudioTranscriptionModel(AiPlatformEnum platform, String apiKey, String url) {
        //noinspection EnhancedSwitchMigration
        switch (platform) {
//            case TONG_YI:
//                return buildTongYiImagesModel(apiKey);
...
//                return buildZhiPuAiImageModel(apiKey, url);
            case OPENAI:
                return buildOpenAiAudioTranscriptionModel(apiKey, url);
//            case STABLE_DIFFUSION:
//                return buildStabilityAiImageModel(apiKey, url);
            default:
                throw new IllegalArgumentException(StrUtil.format("未知平台({})", platform));
    ...
    @Override
    public OpenAiAudioApi getOrCreateOpenAiAudioApi(String openAiToken, String url) {
        url = StrUtil.blankToDefault(url, ApiUtils.DEFAULT_BASE_URL);
        OpenAiAudioApi openAiApi = new OpenAiAudioApi(url, openAiToken, RestClient.builder(), RetryUtils.DEFAULT_RESPONSE_ERROR_HANDLER);
        return openAiApi;
    }
    ...
    private static OpenAiChatModel buildOpenAiChatModel(String openAiToken, String url) {
        ...
        OpenAiChatOptions options = new OpenAiChatOptions.Builder().withModel(OpenAiApi.ChatModel.GPT_4_O).build();
        return new OpenAiChatModel(openAiApi, options);
    }

    private static OpenAiChatModel buildOpenAiChatModel(String openAiToken, String url, OpenAiChatOptions options) {
        url = StrUtil.blankToDefault(url, ApiUtils.DEFAULT_BASE_URL);
        OpenAiApi openAiApi = new OpenAiApi(url, openAiToken);
        return new OpenAiChatModel(openAiApi, options);
    }
    ...
    private OpenAiAudioTranscriptionModel buildOpenAiAudioTranscriptionModel(String openAiToken, String url) {
        url = StrUtil.blankToDefault(url, ApiUtils.DEFAULT_BASE_URL);
        OpenAiAudioApi openAiApi = new OpenAiAudioApi(url, openAiToken, RestClient.builder(), RetryUtils.DEFAULT_RESPONSE_ERROR_HANDLER);
        OpenAiAudioTranscriptionModel openAiAudioTranscriptionModel = new OpenAiAudioTranscriptionModel(openAiApi);
