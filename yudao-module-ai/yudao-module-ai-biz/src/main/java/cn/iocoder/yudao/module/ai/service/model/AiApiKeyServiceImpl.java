...
public class AiApiKeyServiceImpl implements AiApiKeyService {
    ...
    @Override
    public ChatModel getChatModel(AiPlatformEnum platform) {
    ...
    @Override
    public OpenAiAudioTranscriptionModel getOpenAiAudioTranscriptionModel(AiPlatformEnum platform) {
        AiApiKeyDO apiKey = apiKeyMapper.selectFirstByPlatformAndStatus(platform.getPlatform(), CommonStatusEnum.ENABLE.getStatus());
        if (apiKey == null) {
            throw exception(API_KEY_IMAGE_NODE_FOUND, platform.getName());
        }
        return modelFactory.getOrCreateOpenAiAudioTranscriptionModel(platform, apiKey.getApiKey(), apiKey.getUrl());
    }

    @Override
    public OpenAiAudioApi getOpenAiAudioApi(AiPlatformEnum platform) {
        AiApiKeyDO apiKey = apiKeyMapper.selectFirstByPlatformAndStatus(platform.getPlatform(), CommonStatusEnum.ENABLE.getStatus());
        if (apiKey == null) {
            throw exception(API_KEY_IMAGE_NODE_FOUND, platform.getName());
        }
        return modelFactory.getOrCreateOpenAiAudioApi(apiKey.getApiKey(), apiKey.getUrl());
    }
